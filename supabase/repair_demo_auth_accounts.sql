-- Repair demo auth accounts if /auth/v1/token returns:
-- { code: "unexpected_failure", message: "Database error querying schema" }
--
-- Run order in Supabase SQL Editor:
-- 1) Run ensure_demo_roles.sql in its own execution.
-- 2) Run this file in a new execution.
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      UPDATE auth.users
      SET encrypted_password = extensions.crypt('TestPass!2345', extensions.gen_salt('bf', 10)),
          email_confirmed_at = COALESCE(email_confirmed_at, now()),
          raw_app_meta_data = jsonb_build_object('provider', 'email', 'providers', ARRAY['email']),
          raw_user_meta_data = jsonb_build_object('username', account.username),
          updated_at = now()
      WHERE id = account_id;
    END IF;
    ...
    WHERE user_id = account_id
       OR provider_id = account_id::text
       OR lower(identity_data->>'email') = lower(account.email);
